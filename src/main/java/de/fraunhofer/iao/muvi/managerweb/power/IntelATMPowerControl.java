...
public class IntelATMPowerControl {
	...
	private String returnMessage = "ConnectionLost";
	private boolean togglePower = false;
	
	private Thread powerState = new Thread(new Runnable() {
		@Override
		public void run() {
			int powerState;
			...
			try {
				...
				returnMessage= translatePowerState(powerState);
			}
			catch (Exception e) {
				log.error("Exception: " + e.getMessage());
				returnMessage =  "ConnectionLost";
			}
		}
	});
	
	private Thread powerUpThread = new Thread(new Runnable() {
		@Override
		public void run() {
	...
	/**
	 * will execute the function in a parallel thread with a short timeout
	 * @param parameter getPowerState|powerUp
	 */

	public void startThread(String  parameter) {
		if(parameter.matches("getPowerState") ){
			powerState.start();
			try {
				powerState.join(1000);
			} catch (InterruptedException e) {
				log.error("Thread for "+parameter+" interrupted");
			}
		}
		if(parameter.matches("powerUp") ){
			powerUpThread.start();
			try {
				powerUpThread.join(3000);
			} catch (InterruptedException e) {
				log.error("Thread for "+parameter+" interrupted");
			}
		}
